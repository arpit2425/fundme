...
// const idl= require("../target/idl/fundus.json")
...
describe('fundus', () => {
  ...
  const program= anchor.workspace.Fundus as anchor.Program<Fundus>;
  ...
  it('creates a campaign', async () => {
    ...
    const goal = new anchor.BN(5500*LAMPORTS_PER_SOL)
  ...
  it('updates a campaign', async () => {
    ...
    const newGoal = new anchor.BN(7500*LAMPORTS_PER_SOL) // Updated goal
  ...
  it('donate to campaign', async () => {
    ...
    console.log("donar amount before :",donorBefore)
    console.log("campaign before :", campaignBefore)
    console.log("cid:",CID)

    const donation_amount = new anchor.BN(Math.round(4 * LAMPORTS_PER_SOL)) // 1.5 SOL in lamports
    const tx = await program.rpc.donate(CID, donation_amount, {
      accounts: {
       donar:donor.publicKey,
        campaign: campaignPda,
  ...
  it('updates platform settings', async () => {
    ...
    console.log(
      ...
      (programStateBefore.platformFee).toString()
    )
    ...
    console.log(
      'Program State After:',
      (programStateAfter.platformFee ).toString()
    )
  })
})
